...
namespace Paint.ScanLine
{
    class ScanLine
    {
        ...
        public void ScanLinePolygonFill(List<Point> Q, OpenGL g, int step)
        {
            this.step = step;
            this.g = g;
        ...
        private void ProcessScanLineFill(List<EDGE>[] NET, int ymin, int ymax)
        {
            ...
            for (int y = ymin; y < ymax; y += step)
            {
                ...
                {
                    //g.DrawString((((int)AET[i].xi) / step * step).ToString() + " ", new Font("Microsoft Yahei", 6), blue, new Point(400 + i * 24, y));
                }
        ...
        private void FillAetScanLine(ref List<EDGE> AET, int y)
        {
            ...
            {
                int from = ((int)AET[i].xi + step) / step * step;
                int to = ((int)(AET[i + 1].xi + step / 2)) / step * step;
                if (from < to)
                {
                    g.Begin(OpenGL.GL_LINES);
                    g.Vertex(from - step / 2, y - step / 2);
                    g.Vertex(to - step / 2, y - step / 2);
                    g.End();
                }
            }
        }
